...
pub fn convert(source: &Path, output: &Path, clean: bool) -> Result<usize, String> {
    ...
    write_file(&output.join("README.md"), "# Generated PDF documentation\n\nEach directory contains extracted source text (`document.md`) and categorized structured tables (`tables/`). `manifest.json` lets agents locate a table by source document, page, category, and table file. Rebuild with `bazel run //internal/docs_tool:pdf_to_markdown -- --clean`.\n")?;
    Ok(pdfs.len())
}
