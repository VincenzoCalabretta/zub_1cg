...
extern char _binary_hello_world_bin_start[];
extern char _binary_hello_world_bin_end[];
...
void main(void)
{
    uint32_t *src = (uint32_t *)_binary_hello_world_bin_start;
    uint32_t count = (uint32_t)(uint64_t)(_binary_hello_world_bin_end - _binary_hello_world_bin_start);
    count = (count + 3) / 4;
