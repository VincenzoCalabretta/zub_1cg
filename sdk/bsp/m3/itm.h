...
#define M3_ITM_TCR_SYNCENA (1u << 2) /* Enables ITM sync packets -- see M3_DWT_CTRL_SYNCTAP. */
...
/* TRM 11.2.2: "You must enable synchronization packets in the DWT to
 * provide synchronization for the formatter" -- the TPIU formatter's Full
 * Sync Packet (0xFFFFFF7F, what orbtrace_tpiu_demux.sv's sync_window
 * searches for) is generated by DWT-driven synchronization, not emitted
 * periodically on its own. Without this, the formatter emits only
 * half-sync/idle frames forever and orbtrace can never establish frame
 * alignment, regardless of how much real ITM traffic is flowing -- see
 * M3_TRACE_VERIFICATION_PLAN.md's 2026-08-18 root-cause finding. */
#define M3_DWT_CTRL (*(volatile uint32_t *)0xE0001000UL)
#define M3_DWT_CTRL_CYCCNTENA (1u << 0)
#define M3_DWT_CTRL_SYNCTAP_SHIFT 10u /* 0=off, 1=CYCCNT[24], 2=CYCCNT[26], 3=CYCCNT[28] */
#define M3_DWT_CYCCNT (*(volatile uint32_t *)0xE0001004UL)
...
/* Mode[1:0]: 00=normal, 01=integration test, 10=integration data test (TRM
 * 11.3.9 -- "trace output is disabled, and data can be read directly from
 * each input port using the integration data registers"). Never written by
 * m3_itm_init(); reading it back is a cheap way to rule in/out whether the
 * TPIU is stuck outside normal mode, independent of what CSPSR/SPPR select. */
#define M3_TPIU_ITCTRL (*(volatile uint32_t *)0xE0040F00UL)
...
static inline void m3_itm_init(uint32_t port_width_select) {
    ...
    /* DWT sync tap 1 = CYCCNT[24], the shortest available period -- the
     * formatter's Full Sync Packet is DWT-synchronization-driven, not
     * emitted periodically on its own (TRM 11.2.2); without this the
     * demux can never find frame alignment. See M3_DWT_CTRL's comment. */
    M3_DWT_CTRL = M3_DWT_CTRL_CYCCNTENA | (1u << M3_DWT_CTRL_SYNCTAP_SHIFT);
    M3_ITM_TCR = M3_ITM_TCR_ITMENA | M3_ITM_TCR_SYNCENA | M3_ITM_TCR_TXENA |
                 (1u << M3_ITM_TCR_TRACEBUSID_SHIFT);
}
